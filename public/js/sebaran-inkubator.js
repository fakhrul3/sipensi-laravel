document.addEventListener("DOMContentLoaded", () => {
  ...
  // Data dari controller (kalau kosong pakai fallback dummy)
  const data =
    window.SEBARAN_INKUBATOR_DATA && window.SEBARAN_INKUBATOR_DATA.length
      ...
      : [
          ...
          { id: 5, name: "Jambi", latitude: -1.4852, longitude: 102.438, total: 5 },
          ...
          { id: 8, name: "Lampung", latitude: -5.428, longitude: 105.2619, total: 24 },
          ...
          { id: 16, name: "Banten", latitude: -6.4058, longitude: 106.064, total: 36 },
          ...
          { id: 21, name: "Kalimantan Tengah", latitude: -2.2102, longitude: 113.92, total: 4 },
          ...
          { id: 26, name: "Sulawesi Tengah", latitude: -1.43, longitude: 121.4456, total: 10 },
          ...
          { id: 33, name: "Papua Barat", latitude: -0.8615, longitude: 134.062, total: 1 },
  ...
  // URL tujuan "Lihat selengkapnya"
  const lembagaUrl =
    window.SIPENSI && window.SIPENSI.lembagaUrl
      ? window.SIPENSI.lembagaUrl
      : "/lembaga-inkubator";
  ...
  data.forEach((item) => {
    ...
    const provinsi = item.name || item.provinsi || "Provinsi";
    const jumlah = Number(item.total ?? item.jumlah ?? 0);

    const marker = L.marker([lat, lng], { icon }).addTo(map);

    marker.bindPopup(`
      <div class="sipensi-popup">
        <div class="popup-title">${provinsi}</div>
        <div class="popup-count">${jumlah} Inkubator</div>
        <a class="popup-link" href="${lembagaUrl}">Lihat selengkapnya</a>
      </div>
    `);
  });

  // Biar map gak “blank” kalau dibuka dari posisi scroll bawah / tab inactive
  // (kadang Leaflet butuh invalidateSize)
  setTimeout(() => {
    map.invalidateSize();
  }, 150);
});
