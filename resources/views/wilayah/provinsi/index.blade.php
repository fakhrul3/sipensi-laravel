...
<div class="admin-table-container">
    <div class="admin-table-header">
        <h5 class="admin-table-title">Daftar Provinsi</h5>
        <div class="admin-table-actions">
            <button type="button" class="admin-btn-add" data-bs-toggle="modal" data-bs-target="#provinsiModal" onclick="openProvinsiModal()">
    ...
    <table id="provinsiTable" class="table table-striped table-bordered">
        <thead>
            <tr>
                <th>NO</th>
                <th>NAMA PROVINSI <i class="fas fa-sort"></i></th>
                <th>KODE PROVINSI <i class="fas fa-sort"></i></th>
                ...
                <tr>
                    ...
                    <td>
                        <div class="action-buttons">
                            <button type="button" class="btn-action btn-action-edit" title="Edit" onclick="editProvinsi({{ $provinsi->id }})">
                                <i class="fas fa-pencil-alt"></i>
                            </button>
                            <button type="button" class="btn-action btn-action-delete" title="Delete" onclick="deleteProvinsi({{ $provinsi->id }})">
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close" onclick="closeProvinsiModal()"></button>
            </div>
            <form id="provinsiForm">
                <div class="modal-body">
                    <input type="hidden" id="provinsi_id" name="id">
                    
                    <div class="mb-3">
                        <label for="kode_provinsi" class="form-label">Kode Provinsi <span class="text-danger">*</span></label>
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal" onclick="closeProvinsiModal()">Batal</button>
...
$(document).ready(function() {
    ...
    $('#provinsiModal').on('hidden.bs.modal', function() {
...
function openProvinsiModal() {
    ...
    const modalElement = document.getElementById('provinsiModal');
...
function editProvinsi(id) {
    $.ajax({
        url: '{{ route("wilayah.provinsi.show", ":id") }}'.replace(':id', id),
        method: 'GET',
        success: function(response) {
            if (response.success) {
                $('#provinsi_id').val(response.data.id);
                $('#kode_provinsi').val(response.data.kode_provinsi);
...
function saveProvinsi() {
    ...
    const url = id 
        ? '{{ route("wilayah.provinsi.update", ":id") }}'.replace(':id', id)
        : '{{ route("wilayah.provinsi.store") }}';
...
function deleteProvinsi(id) {
    ...
    $.ajax({
        url: '{{ route("wilayah.provinsi.destroy", ":id") }}'.replace(':id', id),
        method: 'DELETE',
        headers: {
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        },
        success: function(response) {
            if (response.success) {
                alert(response.message || 'Provinsi berhasil dihapus');
                location.reload();
            }
        },
        error: function(xhr) {
