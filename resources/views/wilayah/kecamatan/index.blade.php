...
<div class="admin-table-container">
    <div class="admin-table-header">
        <h5 class="admin-table-title">Daftar Kecamatan</h5>
        <div class="admin-table-actions">
            <button type="button" class="admin-btn-add" data-bs-toggle="modal" data-bs-target="#kecamatanModal" onclick="openKecamatanModal()">
    ...
    <table id="kecamatanTable" class="table table-striped table-bordered">
        <thead>
            <tr>
                <th>NO</th>
                <th>NAMA KECAMATAN <i class="fas fa-sort"></i></th>
                <th>KODE KECAMATAN <i class="fas fa-sort"></i></th>
                <th>KABUPATEN ID <i class="fas fa-sort"></i></th>
                <th>AKSI</th>
            </tr>
        </thead>
        <tbody>
                ...
                <tr>
                    ...
                    <td>
                        <div class="action-buttons">
                            <button type="button" class="btn-action btn-action-edit" title="Edit" onclick="editKecamatan({{ $kecamatan->id }})">
                                <i class="fas fa-pencil-alt"></i>
                            </button>
                            <button type="button" class="btn-action btn-action-delete" title="Delete" onclick="deleteKecamatan({{ $kecamatan->id }})">
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close" onclick="closeKecamatanModal()"></button>
            </div>
            <form id="kecamatanForm">
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        <label for="kode_kecamatan" class="form-label">Kode Kecamatan <span class="text-danger">*</span></label>
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal" onclick="closeKecamatanModal()">Batal</button>
...
function openKecamatanModal() {
    ...
    const modalElement = document.getElementById('kecamatanModal');
...
function editKecamatan(id) {
    $.ajax({
        url: '{{ route("wilayah.kecamatan.show", ":id") }}'.replace(':id', id),
        method: 'GET',
        success: function(response) {
            if (response.success) {
                $('#kecamatan_id').val(response.data.id);
                $('#kode_kecamatan').val(response.data.kode_kecamatan);
                $('#kabupaten_id').val(response.data.kabupaten_id);
...
// Save kecamatan (create/update)
function saveKecamatan() {
    const formData = {
        kode_kecamatan: $('#kode_kecamatan').val(),
    ...
    const url = id 
        ? '{{ route("wilayah.kecamatan.update", ":id") }}'.replace(':id', id)
        : '{{ route("wilayah.kecamatan.store") }}';
...
function deleteKecamatan(id) {
    ...
    $.ajax({
        url: '{{ route("wilayah.kecamatan.destroy", ":id") }}'.replace(':id', id),
        method: 'DELETE',
        headers: {
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        },
        success: function(response) {
            if (response.success) {
                alert(response.message || 'Kecamatan berhasil dihapus');
                location.reload();
            }
        },
        error: function(xhr) {
